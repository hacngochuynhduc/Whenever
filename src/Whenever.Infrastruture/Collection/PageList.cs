...
public class PagedList<T> : IPagedList<T>
{
    ...
    internal PagedList(IEnumerable<T> source, int pageIndex, int pageSize, int indexFrom)
    {
        ...
        if (source is IQueryable<T> querable)
        {
            ...
            Items = querable.Skip(PageIndex * PageSize - 1).Take(PageSize).ToList();
        }
        else
        {
            PageIndex = pageIndex;
            ...
            TotalPages = (int)Math.Ceiling(TotalCount / (double)PageSize);
            Items = source.Skip(PageIndex * PageSize - 1).Take(PageSize).ToList();
        }
    }
...
internal class PagedList<TSource, TResult> : IPagedList<TResult>
{
        ...
        int pageIndex, int pageSize, int indexFrom)
    {
        ...
        if (source is IQueryable<TSource> querable)
        {
            ...
            TotalCount = querable.Count();
            TotalPages = (int)Math.Ceiling(TotalCount / (double)PageSize);
            var items = querable.Skip(PageIndex * PageSize - 1).Take(PageSize).ToArray();
        ...
        else
        {
            ...
            var items = source.Skip(PageIndex * PageSize - 1).Take(PageSize).ToArray();
